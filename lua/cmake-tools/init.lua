...
function cmake.generate(opt, callback)
	...
	if presets_file and config.configure_preset then
		...
		if config.executor.name == "terminal" then
			if full_cmd ~= "" then
				full_cmd = full_cmd ..
				" && " .. terminal.prepare_cmd_for_run(const.cmake_command, {}, args)
			else
...
function cmake.quick_build(opt, callback)
	...
	if opt.fargs[1] == nil then
		...
		-- TODO I should add a command to get the cmake command itself. It should use plentry async module.
		...
		return cmake.build({ target = opt.target }, function()
			...
			if full_cmd ~= "" then
				full_cmd = 'cd "'
				    ...
				    .. full_cmd
				    .. " && "
				    .. terminal.prepare_cmd_for_execute(target_path, opt.args, launch_path, opt
				    .wrap_call)
			else
				full_cmd =
